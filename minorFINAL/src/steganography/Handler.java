...
public class Handler extends Thread
{
    ...
    public Handler(WindowAdapter client, short operation)
        {
  this.client= client;
  ...
  MyFileFilter filter1= new MyFileFilter(new String[]
  {"bmp", "jpg", "gif", "tif"}, "Picture files");
  MyFileFilter filter2= new MyFileFilter(new String[]
  {"mp3", "wav", "ram", "wma"}, "Audio files");
  MyFileFilter filter3= new MyFileFilter(new String[]
  {"mpg", "wmv", "dat"}, "Video files");
  ...
  switch(operation)
  {
   case EMBED_MESSAGE: new EmbedMessage(this); break;
   case EMBED_FILE:  new EmbedFile(this);  break;
   case RETRIEVE_MESSAGE:
     ...
     if(steg.isFile())
      ...
      if(!steg.isValid())
       ...
       new Retrieve(steg, Retrieve.RETRIEVE_MESSAGE);
     break;
   case RETRIEVE_FILE:
     steg= new SteganoInformation(masterFile);
     if(steg.isFile())
      ...
      if(!steg.isValid())
       ...
       new Retrieve(steg, Retrieve.RETRIEVE_FILE);     
  }
 }
  ...
  if(!file.exists())
  {
   JOptionPane.showMessageDialog(null, "File "+ file.getName()+ 
           " does not exist!", "Inexistent file!", 
  ...
  if(password.length()<8)
   JOptionPane.showMessageDialog(null, 
           "This was not the right password!",
  ...
  else
  {
   ...
   try
   {
    DataInputStream in= new DataInputStream(
            new FileInputStream(steg.getFile()));
    in.read(byteArray, 0, fileSize);
    in.close();

    Cipher cipher= Cipher.getInstance("DES");
    cipher.init(Cipher.DECRYPT_MODE, 
            new SecretKeySpec(password.substring(0, 8)
                    .getBytes(), "DES"));
   ...
   frame.getContentPane().add(new JScrollPane(
           new JLabel(new ImageIcon(byteArray))));
