...
public class EmbedMessage extends JFrame implements ActionListener, ItemListener
{
    Handler          client;
    private JLabel          lblMaster, lblOutput,
                            lblMasterSize, lblOutputSize, lblMessage;;
    private JLabel          lblMasterFileSize,
                            lblOutputFileSize, lblStatus, lblFiller[];
    ...
    private JButton         btnGo, btnHelp, btnCancel,
                            btnChangeMasterFile, btnChangeOutputFile;
 public EmbedMessage(Handler client)
    {
  super("Embedding message - Steganograph ");
  ...
  txtMasterFile= new JTextField(client.getMasterFile().getName());
  txtMasterFile.setForeground(Color.BLUE);
  txtMasterFile.setEditable(false);
  lblMasterFileSize= new JLabel(""+ 
          client.getMasterFile().length()/1024+ " Kb");
   lblMasterFileSize.setForeground(Color.red);
  txtOutputFile= new JTextField(client.getOutputFile().getName());
  txtOutputFile.setForeground(Color.BLUE);
  txtOutputFile.setEditable(false);
  lblOutputFileSize= new JLabel(lblMasterFileSize.getText());
  lblOutputFileSize.setForeground(Color.red);
  lblMessage= new JLabel("Message:");
  lblStatus= new JLabel("Minimum required size of the Master file: 32Kb");
  ...
  gbc.gridx= 2; gbc.gridy= 2; gbl.setConstraints
        (btnChangeMasterFile, gbc);
  ...
  panelFiles=  UtilityOperations.createBorderedPanel
        (panelFiles, "Files", 19, 3);

  lblCompression= new JLabel("Compression level");
  lblPassword= new JLabel("Password (Minimum 8 chars)");
  ...
  panelCompression2.add(new JLabel("Low"));
  panelCompression2.setForeground(Color.BLUE);
  panelCompression2.add(sliderCompression);
  panelCompression2.add(new JLabel("High"));
  ...
  panelCompression= UtilityOperations.
          createBorderedPanel(panelCompression, "Compression", 3, 3);
  JPanel panelEncryption= new JPanel();
  new BoxLayout(panelEncryption, BoxLayout.X_AXIS);
  panelEncryption.add(chkEncrypt);
  panelEncryption.add(lblPassword);
  panelEncryption.add(txtPassword);
  panelEncryption= UtilityOperations.
          createBorderedPanel(panelEncryption, "Encryption", 3, 15);
  ...
  gbc.gridy= 2; gbc.anchor= gbc.WEST; 
  gbl.setConstraints(lblMessage, gbc);
  panelText.add(lblMessage);
  gbc.gridy= 3; gbc.anchor= gbc.CENTER; 
  ...
  gbc.anchor= gbc.CENTER; gbc.gridx= 1; gbc.gridy= 1; 
 ...
 public void actionPerformed(ActionEvent e)
  ...
  if(source== btnGo)
  {
   ...
   if(Steganograph.embedMessage(client.getMasterFile(),
           client.getOutputFile(), txtMessage.getText(),
           compression, password))
    JOptionPane.showMessageDialog(this, 
            Steganograph.getMessage(), "Operation Successful", 
            JOptionPane.INFORMATION_MESSAGE);
   else
    JOptionPane.showMessageDialog(this,
            Steganograph.getMessage(), "Operation Unsuccessful",
