...
public class Steganograph 
{
    ...
    public static final byte[]  VERSION_BYTE= {'0','0','0'};
 ...
 // Embeds a message into a Master file
    public static boolean embedMessage(
            File masterFile, File outputFile, 
  ...
  try
  {
   ...
   masterExtension= fileName.substring(
   ...
   if(features== CUM || features== CEM)
   {
    ...
    compressionRatio= (short) ((double)
            messageArray.length / (double)messageSize * 100.0);
    messageSize= messageArray.length;
   }
   ...
   if(features== UEM || features== CEM)
   {
    ...
    SecretKeySpec spec= new SecretKeySpec(
            password.substring(0, 8).getBytes(), "DES");
 ...
 // Retrieves an embedded message from a Master file
    public static String retrieveMessage(
         SteganoInformation info, String password)
        {
  String messg= null;
  ...
  try
  {
   ...
   messg= new String(SteganoInformation.
           byteToCharArray(messageArray));
  }
  catch(Exception e)
  {
   message= "Oops!!\n Error: "+ e;
 ...
 // Embeds a file into a Master file
    public static boolean embedFile(File masterFile,
        File outputFile, File dataFile, int compression, String password)
         {
  ...
  message= "File '"+ dataFile.getName()+ "'"
          + " embedded successfully in file '"+
          outputFile.getName()+ "'.";
  return true;
 }

 // Retrieves an embedded file from a Master file
    public static boolean retrieveFile(
         SteganoInformation info, String password, boolean overwrite)
         {
  File dataFile= null;
  features= info.getFeatures();
  try
 ...
 // Method used to read bytes into the output array
  ...
  for(int i=0; i< size; i++)
  {
   ...
   inputOutputMarker++;
	...
	{
		new HTMLFrame("https://en.wikipedia.org/wiki"
                        + "/Steganography", false);
	}
